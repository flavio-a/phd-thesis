...
\begin{definition}[Local completeness, cf.\cite{BGGR21}]
	...
	\]
\end{definition}
While local completeness is a weaker property, so it holds more often, it has a crucial drawback with respect to global completeness. In fact, the latter can be proved compositionally on the command only \cite{GLR15}, while the former needs also the input to each fragment of the program. To help cope with this issue, the authors observe that local completeness enjoys an ``abstract convexity" property: whenever it holds on a concrete point $c$ it does so also on all points $d$ between $c$ and its abstraction $A(c)$. This property became crucial in the design of the $\LCLA$ proof system, whose goal is to prove a program is locally complete on a given input.

$\LCLA$ depends on an abstract domain $A$, and a triple $\lcl{A}{P}{\regr}{Q}$ provable in the logic ensures three conditions:
\begin{enumerate}
...
\begin{figure*}[t]
	...
	%	\vspace{-4ex}
\end{figure*}
The proof system is defined in Figure~\ref{fig:sota:lcla-rules}. It is a logic of under-approximations, much like IL (actually IL is a special case of $\LCLA$), but with one additional constraint: the under approximation $Q$ must have the same abstraction of the concrete semantics $\denot{\regr} P$, as for instance explicitly required in rule \lrule{relax}. This, by the abstract convexity property mentioned above, means that local completeness of $\denot{\regr}$ on the \emph{under-approximation} $P$ of the concrete store is enough to prove local completeness.
This way, $\LCLA$ exploits the interaction of over- and under-approximation. The latter is used to ensure the abstraction is locally complete, ie. guarantees precision of the over-approximation. Conversely, the presence of the abstraction in rule \lrule{iterate} speeds up the computation as it allows to stop as soon as $A(P)$ is an abstract loop invariant, not having to deal with (possible infinitary) concrete invariants.

Formally, the three key properties (1--3) listed above are formalized by the following soundness result:
...
In $\LCLA$, being able to prove any triple $\lcl{A}{P}{\regr}{Q}$ allows to show both correctness and incorrectness of $\regr$. However, if $\regr$ is not locally complete on $P$, or more in general any of the local completeness proof obligations introduced by rule \lrule{transfer} (the only axiom of the logic) fails, the proof cannot be completed.
