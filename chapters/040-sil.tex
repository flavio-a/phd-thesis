...
// assert(x != 2000000)
	...
	With this triple for the loop, we can prove for the whole program the triple $\angletriple{\true}{\mathsf{rloop0}}{Q_{2M}}$ using \silrule{seq} and \silrule{atom}. The full derivation is in Figure~\ref{fig:sil:example-derivation-2}.
...
\begin{figure}[t]
	...
	\begin{framed}
		...
		\begin{array}{cc}
			\infer[\silrule{skip}]
			{\angletriple{\emp}{\code{skip}}{\emp}}
			{}
			\;                             &
			\infer[\silrule{assign}]
			{\angletriple{q[a / x]}{\code{x := a}}{q}}
			{}
			\\[7.5pt]
			\infer[\silrule{assume}]
			{\angletriple{q \land b}{\code{b?}}{q}}
			{}
			\;                             &
			\infer[\silrule{alloc}]
			{\angletriple{\emp}{\code{x := alloc()}}{x \mapsto v}}
			{}
			\\[7.5pt]
			\infer[\silrule{free}]
			{\angletriple{x \mapsto -}{\code{free(x)}}{x \dealloc}}
			{}
			\;                             &
			\infer[\silrule{load}]
			{\angletriple{y \mapsto a \andsep q[a / x]}{\code{x := [y]}}{y \mapsto a \andsep q}}
			{x \notin \fv(a)}
			\\[7.5pt]
			\infer[\silrule{store}]
			{\angletriple{x \mapsto -}{\code{[x] := y}}{x \mapsto y}}
			{}
			\\[7.5pt]
			\hline\hline                   &                                                               \\[-2pt]
			\infer[\silrule{exists}]
			{\angletriple{\exists x. p}{\regr}{\exists x. q}}
			{\angletriple{p}{\regr}{q}     & x \notin \fv(\regr)}
			\;                             &
			\infer[\silrule{frame}]
			{\angletriple{p \andsep t}{\regr}{q \andsep t}}
			{\angletriple{p}{\regr}{q}     & \fv(t) \cap \modified(\regr) = \emptyset}
			\\[7.5pt]
			\hline\hline                   &                                                               \\[-2pt]
			\infer[\silrule{cons}]
			{\angletriple{p}{\regr}{q}}
			{p \Rightarrow p'              & \angletriple{p'}{\regr}{q'}               & q' \Rightarrow q}
			\;                             &
			\infer[\silrule{seq}]
			{\angletriple{p}{\regr_1; \regr_2}{q}}
			{\angletriple{p}{\regr_1}{t}   & \angletriple{t}{\regr_2}{q}}
			\\[7.5pt]
			\infer[\silrule{choice}]
			{\angletriple{p_1 \lor p_2}{\regr_1 \regplus \regr_2}{q}}
			{\angletriple{p_1}{\regr_1}{q} & \angletriple{p_2}{\regr_2}{q}}
			\;                             &
			\infer[\silrule{iter}]
			{\angletriple{\exists n. q(n)}{\regr^\kstar}{q(0)}}
			{\forall n \ge 0 \;\; \angletriple{q(n+1)}{\regr}{q(n)}}
			\\[7.5pt]
			\hline\hline                   &                                                               \\[-2pt]
			\infer[\silrule{empty}]
			{\angletriple{\false}{\regr}{q}}
			{}
			\;                             &
			\infer[\silrule{disj}]
			{\angletriple{p_1 \lor p_2}{\regr}{q_1 \lor q_2}}
			{\angletriple{p_1}{\regr}{q_1} & \angletriple{p_2}{\regr}{q_2}}
			\\[7.5pt]
			\infer[\silrule{iter0}]
			{\angletriple{q}{\regr^{\kstar}}{q}}
...
\begin{figure}[t]
	...
	\end{align*}
	\caption{The proof sketch of $\angletriple{(v \mapsto z \andsep z \mapsto - \andsep \true)}{\mathsf{rclient}}{x \dealloc \andsep \true}$ (best read bottom-up). We write only the strengthened conditions obtained using \silrule{cons}, and \underline{underline} the post of the rule for the current atomic command. Everything else is shared between pre and post using \silrule{frame}.}
...
Using this, for any atomic command $\regc \in \Cmdh$ we compute an assertion $t$ whose semantics is precisely the weakest possible pre $\bwsem{\regc} \asldenot{q}$, and prove the triple $\angletriple{t}{\regc}{q}$. Then, we extend this result to any program not containing $\kstar$, and conclude using the oracle and \silrule{cons} to prove the triple for any $p$ that implies $t$.
