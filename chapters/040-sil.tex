...
\begin{example}\label{ex:sil-derivation-2}
...
// assert(x != 2000000)
	...
	We let $R_{2M} \eqdef (x = 2000000 \land n \le 0)$ and $T_{2M} \eqdef (x + n = 2000000 \land n > 0)$. It is straightforward to prove $\angletriple{T_{2M}}{\regr_w}{R_{2M}}$ via \silrule{seq} and \silrule{atom}. Given this triple, we can unroll the loop once and prove the same triple for $\regr_w^{\kstar}$:
	\[
	...
	With this triple for the loop, we can prove for the whole program the triple $\angletriple{\true}{\mathsf{rloop0}}{Q_{2M}}$ using \silrule{seq} and \silrule{atom}. The full proof is in the Appendix, Figure~\ref{fig:app:sil-example-derivation-2}.
...
Rule \silrule{disj} allows to split the analysis and join the results, just like HL and IL. However, while a corresponding rule \hlrule{conj} which perform intersection is sound for HL, it is unsound for both IL and SIL (see Example~\ref{ex:il-no-strongest-pre}).

\section{Relations Among Logics}\label{sec:sil:comparison}
\todo[inline]{Check this whole section because rn is quite bad (also add some stuff for NC in the background and the appendix)}
...
\subsubsection*{Weakest/Strongest Conditions}
...
%
%To investigate the existence of weakest/strongest conditions, we build on the consequence rules of the four logics:
%each consequence rule explains how pre and post can be weakened/strengthened.
...
For instance, given two HL preconditions $P_1$ and $P_2$ for the same post $Q$ (i.e., $\overtriple{P_1}{\regr}{Q}$ and $\overtriple{P_2}{\regr}{Q}$) also their union is a pre for $Q$, i.e., $\overtriple{P_1 \cup P_2}{\regr}{Q}$ using \hlrule{disj}. However, given two IL triples $\undertriple{P_1}{\regr}{Q}$ and $\undertriple{P_2}{\regr}{Q}$, in general $\fwsem{\regr} (P_1 \cap P_2) \nsupseteq Q$ and $\undertriple{P_1 \cap P_2}{\regr}{Q}$ is not valid.
...
\todo{Move this around}
Rule \silrule{disj} allows to split the analysis and join the results, just like HL and IL. The validity of such a rule relies on the fact that collecting semantics (both forward and backward) are additive. However they are not co-additive, so the corresponding rule \silrule{conj} which perform intersection of pre/post pairs is sound for HL but not for IL and SIL.
...
\subsubsection*{Termination and Reachability}
%Note that the forward semantics does not account for divergent computations. This means that if $\sigma_1$ and $\sigma_2$ can reach the same set of final states, but $\sigma_1$ has a divergent computation while $\sigma_2$ does not, they have the same forward semantics. The only kind of divergence this definition can distinguish is whether a state $\sigma$ \emph{always} diverges ($\fwsem{\regr}\sigma =\emptyset$) or \emph{may} terminate ($\fwsem{\regr}\sigma \neq\emptyset$). Analogously, the backward semantics only distinguishes between final states which are reachable or not.
%
%As we already noted in Section~\ref{sec:bwsem}, termination and reachability are two sides of the same coin when switching from forward to backward reasoning.
%For HL, given the definition of collecting semantics we can only distinguish a precondition which always causes divergence. This is because the collecting semantics ``does not see" divergent computations, so we only know whether $Q$ is empty or not, which correspond to guaranteed divergence. Moreover, because of the over-approximation, a non empty $Q$ does not mean there are finite executions, as those may be introduced by the approximation.
%Dually, NC cannot  say much about reachability of $Q$ other than the fact that it is unreachable if we find false as a necessary precondition for it.
%On the other hand, under-approximation has much stronger guarantees on divergence/reachability. Taking IL, any triple $\undertriple{P}{\regr}{Q}$ ensures that all states in $Q$ are reachable from states in $P$ which, in particular, means every state in $Q$ is reachable. Dually, a SIL triple $\angletriple{P}{\regr}{Q}$ means that all states in $P$ have a convergent computation (which ends in a state in $Q$). This observation motivates the choice of a forward (resp. backward) rule for iteration in IL (resp. SIL): a backward (resp. forward) rule would be more involved as it would need to prove reachability of all points in the postcondition (resp. precondition). Instead, the forward rule of IL (resp. backward rule of SIL) ensures reachability (resp. termination) by construction, as it build $Q$ only from points which are known to be reachable (resp. terminating) by execution of the loop.
...
For HL
%, given the definition of collecting semantics,
