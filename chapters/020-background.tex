...
\end{align*}
The first construct describe standard first-order logic. The last three describes heap: $\emp$ is satisfied only by the empty heap, $\code{a}_1 \mapsto \code{a}_2$ is satisfied by heaps with a single location, corresponding to the evaluation of $\code{a}_1$ and containing the value $\code{a}_2$, and $\andsep$ is the separating conjunction described above.
\todo{What do I want to do say about SL?}
