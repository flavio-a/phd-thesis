...
\]
In other words, a valid IL triple $\vDash \undertriple{P}{\regr}{Q}$ means that all states in $Q$ are reachable when executing $\regr$ starting from states in $P$. This can be better seen from the equivalent first-order formula
\[
...
\begin{theorem}[IL is sound and complete~\cite{OHearn20}]
	...
	\[
	\vdash\undertriple{P}{\regr}{Q} \iff \vDash\undertriple{P}{\regr}{Q}
	\]
\end{theorem}
...
// assert(z != 42)
	...
	The valid HL triple
	\[
	\overtriple{ \text{odd}(y) \land \text{even}(x)}{\regr}{Q}
	\]
