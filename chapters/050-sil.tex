...
We remark that this backward semantics $\bwsem{\cdot}$ is not Dijkstra $\wlp[\cdot]$. Instead, it correspond to Hoare's weakest possible precondition \cite{Hoare78}. Particularly, this means that $\fwsem{\regr}$ and $\bwsem{\regr}$ are not adjoints in the sense that $Q \subseteq \fwsem{\regr} P$ if and only if $\bwsem{\regr} Q \subseteq P$. For instance, it's easy to build a counterexample using unreachable states (see Lemma~\ref{lmm:sil:CC-1-monotone}). However, we will show an adjunctive relation between the two in Proposition~\ref{prop:sil:fw-inclusion-negation-bw}.
...
The topmost set of five rules in Figure~\ref{fig:sil:sil-rules} is deliberately minimal: if we remove any rule it is no longer complete. However, there are other valid rules that can be useful in practice, and not all of them can be directly derived from the core rules. Some of them are at the bottom of Figure~\ref{fig:sil:sil-rules}. Note that, even if the rules themselves are not derivable (for instance the \silrule{empty} rule), by completeness of the core rules they don't allow to prove any additional triple.
...
Particularly, this establishes an adjunction between $\fwsem{\regr}$ and $\lnot \circ \bwsem{\regr} \circ \lnot$.
...
The syntax only allows to allocate, free and dereference single variables. To use a value from the heap in an arithmetic $\code{a} \in \AExp$ or Boolean expressions $\code{b} \in \BExp$, it must be assigned to a variable beforehand.

Given a heap command $\regr \in \Regh$, we let $\fv(\regr) \subseteq \Var$ be the set of (free) variables of $\regr$ and $\modified(\regr) \subseteq \Var$ be the set of variables modified by $\regr$ (the latter is a syntactic approximation of the variable possibly modified by the program). The definition of the former is standard, while the latter is defined inductively in Figure~\ref{fig:sil:regh-mod-def}. Note that \code{free(x)} and \code{[x] := y} do not modify \code{x}: they only modify the value \emph{pointed by} \code{x}, not the actual value of \code{x} (the memory address itself).
	...
	\begin{framed}
		\resizebox{\textwidth}{!}{
			...
			\begin{array}{cc}
				...
				{}
				\\[7.5pt]
				\infer[\silrule{alloc1}]
				{\siltriple{\emp}{\code{x := alloc()}}{x \mapsto v}}
				{}
				\;                           &
				\infer[\silrule{assume}]
				{\siltriple{\emp \land b}{\code{b?}}{\emp}}
				{}
				\\[7.5pt]
				\infer[\silrule{alloc2}]
...
Rules \silrule{alloc1} and \silrule{alloc2} allocate a new memory location for $x$: in the former the location is new, in the latter it reuses the previously deallocated location $\beta$. In both variants, if the previous value of $x$ is needed, $x = z$ can be introduced in the premise with \silrule{cons}. Note that, in theory, this doubles the size of the formulae when allocation appears in the program because each allocation could be analysed with both rules. However, in practice, we propose an algorithm that exploits \silrule{alloc2} only, thus avoiding this issue (see Sections~\ref{sec:sil:separation-sil-inference}and \ref{sec:sil:separation-sil-complete}).
...
In Example~\ref{ex:separation-sil-derivation} this algorithm finds precisely the disjunction $x = y \lor x \dealloc$ in the post of \code{free(y)}.
...
\]
Note that this rule is enough to short-circuit sequence and iterations in case an error happens. For instance, for sequence, you can prove $\siltriple{\oktext{ok: p}}{\regr_1}{\ertext{er:q}}$ and then use \silrule{seq} together with the triple $\siltriple{\ertext{er: q}}{\regr_2}{\ertext{er: q}}$ proved via \silrule{er\mbox{-}id}. Similarly for iteration, you can apply rule \silrule{unroll} and use the same derivation for sequence. However, note that this is not useful during a backward analysis: if the postcondition of an iteration is an \erflag{} state, either you skip the loop entirely with \silrule{er\mbox{-}id}, or you try to find an \okflag{} precondition leading to the error. A derivation such as
\[
\infer[\silrule{unroll}]{\siltriple{\oktext{ok: p}}{\regr^{\kstar}}{\ertext{er: q}}}{
	\infer[\silrule{seq}]{\siltriple{\oktext{ok: p}}{\regr^{\kstar}; \regr}{\ertext{er: q}}}{
		\infer[]{\siltriple{\oktext{ok: p}}{\regr^{\kstar}}{\ertext{er: q}}}{\vdots}
		&
		\infer[\silrule{er\mbox{-}id}]{\siltriple{\ertext{er: q}}{\regr}{\ertext{er: q}}}{}
	}
}
\]
is useless. Similarly, it's useless to apply rule \silrule{iter} with $q(1) = q(0) = \ertext{er: q}$; instead, lower indexes in $q(n)$ by 1 (and drop $q(0)$).
...
\subsection{Relative completeness of Separation SIL}\label{sec:sil:separation-sil-complete}
The proof system in Section~\ref{sec:sil:sepsil-proof-system} is complete for all atomic commands except \code{alloc} because \silrule{alloc1} misses the ability to refer to the specific memory location that was allocated.
The naive solution to add a constraint $x = \beta$ in the post of \silrule{alloc1} makes the frame rule unsound: it would allow to prove, for instance, the invalid triple
\[
\siltriple{\emp \andsep \alpha \mapsto -}{\code{x := alloc()}}{(x \mapsto - \land x = \alpha) \andsep \alpha \mapsto -} \text{.}
\]
To recover the frame rule, just like ISL needs the deallocated assertion in the post~\cite[\S 3]{RBDDOV20}, Separation SIL needs a ``will be allocated'' assertion in the pre. To this end, we use the $\dealloc$ assertion, and change the semantic model to only allocate a memory location that is explicitly $\delta$. Intuitively, this means specifying the resources that will be needed by the program execution: in particular, it lists in the pre the memory locations that will be allocated during program execution. Note that memory locations that will be allocated can be specified as logical variables, so they don't constrain the actual memory layout, but only the amount of locations needed.
We formalize this by letting $\mathit{avail}_h(l) \eqdef (h(l) = \delta)$ in Figure~\ref{fig:sil:ssil-model-commands} and removing rule \silrule{alloc1}: the only valid rule for allocation in this semantics is \silrule{alloc2}. Moreover, we can prove relative completeness~\cite[\S 4.3]{AO19} for loop-free programs:
...
To gain confidence in the mechanizability of our results, we developed a proof-of-concept implementation of Separation SIL in OCaml. The prototype is open source and available on GitHub.\footnote{\url{https://github.com/Alex23087/Failure-SSIL-Analyser}} While it is not meant to scale up to real programs, it can derive all the examples in this section and validates our intuition on the automated backward inference of Section~\ref{sec:sil:separation-sil-inference}. The code exploits a routine that follows closely the process described in that section, which is analogous to the proof of relative completeness (Theorem~\ref{th:sil:separation-sil-sequential-complete}). The core algorithms are implemented in $\sim$400 locs; another big chunk of the code is the simplification of logical formulae, whose complexity is not tied to Separation SIL algorithm.
For this implementation, we thank the students and teachers of the Laboratory for Innovative Software 2024 course, Yuri Andriaccio, Samuele Bonini, Andrea Castagna, Marco Antonio Corallo, Andrea Simone Costa, Fabio Federico, Elvis Rossi, Alessandro Scala, Matteo Simone, Chiara Bodei and Gian-Luigi Ferrari.
...
\subsection{Comparisons}\label{sec:sil:separation-sil-comparisons}
There are other logics that address memory errors with an ``incorrectness'' perspective. Here we compare Separation SIL to ISL and Outcome Separation Logic (OSL)~\cite{ZSS24}.

The key difference between Separation SIL and ISL is the same between IL and SIL: ISL is more effective for forward analysis and ensures reachability of all final states; Separation SIL is thought for backward analysis and guarantees that all initial states can reach a final state. For instance, the ISL triple in~\cite{RBDDOV20} for the Example~\ref{ex:separation-sil-derivation} is $\iltriple{v \mapsto z \andsep z \mapsto -}{\mathsf{rclient}}{v \mapsto y \andsep y \mapsto - \andsep x \dealloc}$, which proves that those faulty states are reachable, but tells nothing about which input states actually lead to them. On the other hand, the Separation SIL triple has a more succinct post capturing the error and exposes faulty initial states, but cannot describe which errors are reachable.

The comparison with OSL is more convoluted.
If we consider the separation instance of OL, \cite[Figure~6]{ZDS23} proves essentially the same triple as Example~\ref{ex:separation-sil-derivation} but the deduction process is quite different. OL reasoning is forward oriented, as witnessed by the implication that concludes the proof and by the triple for the $\code{skip}$ branch, whereas Separation SIL rules naturally guides the backward inference.
OSL is more closely related to Separation SIL because it takes pointer programs at its core. For OSL, the authors propose an abduction-based algorithm that produces similar results to Separation SIL. However, the two algorithms differ in their details and work in different settings.
To tackle the issue of \emph{non-locality} of allocation, \cite[\S 2.2]{ZSS24} prescribe that an OSL triple should be valid for \emph{every} possible allocation semantics. This prevents triples from talking too precisely about the result of an allocation, and is available even when the execution model doesn't include nondeterminism. Thus, OSL frame rule works across different execution models, e.g., for probabilistic programs.
On the contrary, Separation SIL model is tailored to nondeterminism and can't be used for other execution models. However, this allows Separation SIL to be more precise about nondeterminism: differently than OSL, Separation SIL is complete and its triples can talk about a \emph{specific} allocation semantics. Thus Separation SIL can prove some special kinds of triples that are not valid in OSL model.

\begin{example}\label{ex:sil:separation-sil-osl}
	\begin{figure}[t]
		{
			\footnotesize
			\begin{align*}
				 & \textcolor{gray}{\text{// attacker setup}}                                                                                                                                  %\\
				 &                                                              & \silexact{ \true }                                                                                           \\
				 & x := \text{alloc}();                                                                                                                                                        %\\
				 &                                                              & \silexact{ x \mapsto - }                                                                                     \\
				 & \text{free}(x);                                                                                                                                                             %\\
				 &                                                              & \silexact{ (x \dealloc) \lor (x \mapsto secret \andsep \true) }                                              \\
				 & \textcolor{gray}{\text{// user code reading the secret}}                                                                                                                    \\
				 & y := \text{alloc}();                                                                                                                                                        %\\
				 &                                                              & \silexact{ (y \mapsto - \andsep x = y) \lor (x \mapsto secret \andsep y \mapsto - \andsep \true) }           \\
				 & [y] := \code{secret};                                                                                                                                                       %\\
				 &                                                              & \silexact{ (x \mapsto secret \andsep x = y) \lor (x \mapsto secret \andsep y \mapsto secret \andsep \true) } \\
				 & \textcolor{gray}{\text{// attacker code leaking the secret}}                                                                                                                \\
				 & \code{leak} := [x]                                                                                                                                                          %\\
				 &                                                              & \silexact{ leak = secret \andsep x \mapsto - }                                                               \\
				 &
				 &                                                              & \silexact{ leak = secret }                                                                                   \\
			\end{align*}
		}
		\caption{Sketch of the derivation for Example~\ref{ex:sil:separation-sil-osl}. Postconditions to each statement are written on the right of the statement itself.}
		\label{fig:sil:separation-sil-security}
	\end{figure}

	C standard allocator reuses previously freed locations, and this can cause security leaks.
	Consider the code in Fig.~\ref{fig:sil:separation-sil-security}: an attacker sets up a memory address by allocating and then freeing it, so that the next user allocation gets exactly that location $\ell$. The user then stores a secret in $\ell$, that the attacker can read. Separation SIL witnesses the error: in the post of \code{y := alloc()} it can be seen that the leaky behaviour is caused by $(x = y)$. In contrast, OSL cannot witness this error: since it occurs only for a specific allocation semantics this triple is not valid.
\end{example}
