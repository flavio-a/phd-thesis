...
\begin{example}[Extensional and intensional properties]\label{ex:lcla:ext-vs-int}
	...
	The program $\regc$ is equivalent to the command $\code{skip}$,\footnote{In this simplified settings with unbound integers. This is not true for machine integers with overflow.} because $\denot{\code{skip}}=\id_{\setZ}$, and thus $\regc$ and $\code{skip}$ are assigned the same bca $\denot{\code{skip}}^{\Sign} = \denot{\regc}^{\Sign} = \id_{\Sign}$.
...
In the following example(s), we pick the refinement by hand just to show how the rules can be useful. We briefly discuss possible directions on automating the refinement choice in Section~\ref{sec:lcla:choose-refinement}, but this is still a largely unexplored question.
...
However, local completeness enjoys abstract convexity (Lemma~\ref{lmm:sota:abstract-convexity}). This suggests that, even if a concrete point $X$ cannot be abstracted exactly, we can use a different point $P$ that can be abstracted such that $P \le X \le A(P)$. Intuitively, if $f$ is locally complete on $P$, abstract convexity implies that $f$ is locally complete on $x$, too. This idea is the basis of the development in this section.
...
\begin{example}
	...
	Note that, while loop unrolling is a standard technique used in abstract analysers, the use of CLCL forces the unrolling until precision is ensured instead of relying on a fixed number of unrolls or some heuristics.
\end{example}
